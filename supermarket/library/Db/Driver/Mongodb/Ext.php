 ...
 * @version		Version 0.2
 */
class Db_Driver_Mongodb_Ext extends Db_Adapter {
    ...
    public function __construct($mongoConfig) {
        if (!class_exists('Mongo')):
            die("Error 500：The MongoDB PECL extension has not been installed or enabled");
        endif;

        $this->connection_string($mongoConfig);
        $this->connect();
    }
    ...
    public function switch_db($database = '') {
        if (empty($database)):
            die("Error 500：To switch MongoDB databases, a new database name must be specified");
        endif;
        ...
        try {
            ...
            die("Error 500：Unable to switch Mongo Databases: {$e->getMessage()}");
        }
    }
    ...
    public function get($collection = "") {
        if (empty($collection))
            die("Error 500：In order to retreive documents from MongoDB, a collection name must be passed");
        $results = array();
...
//return(iterator_to_array($documents));
    ...
    public function count($collection = "") {
        if (empty($collection))
            die("Error 500：In order to retreive a count of documents from MongoDB, a collection name must be passed");
    ...
    public function insert($collection = "", $data = array()) {
        if (empty($collection))
            die("Error 500：No Mongo collection selected to insert into");
        if (count($data) == 0 || !is_array($data))
            die("Error 500：Nothing to insert into Mongo collection or insert is not an array");
        ...
        try {
            ...
            die("Error 500：Insert of data into MongoDB failed: {$e->getMessage()}");
        }
    }
    ...
    public function update($collection = "", $data = array()) {
        if (empty($collection))
            die("No Mongo collection selected to update");
        if (count($data) == 0 || !is_array($data))
            die("Nothing to update in Mongo collection or update is not an array");

        try {
            $this->db->{$collection}->update($this->wheres, array('$set' => $data), array('safe' => TRUE, 'multiple' => FALSE));
            return(TRUE);
        } catch (MongoCursorException $e) {
            die("Update of data into MongoDB failed: {$e->getMessage()}");
        }
    }
    ...
    public function delete($collection = "") {
        ...
        try {
            ...
            die("Delete of data into MongoDB failed: {$e->getMessage()}");
        }
    }
    ...
    public function add_index($collection = "", $keys = array(), $options = array()) {
        if (empty($collection))
            die("No Mongo collection specified to add index to");
        if (empty($keys) || !is_array($keys))
            die("Index could not be created to MongoDB Collection because no keys were specified");
        ...
        if ($this->db->{$collection}->ensureIndex($keys, $options) == TRUE):
            ...
            die("An error occured when trying to add an index to MongoDB Collection");
        endif;
    }
    ...
    public function remove_index($collection = "", $keys = array()) {
        if (empty($collection))
            die("No Mongo collection specified to remove index from");
        if (empty($keys) || !is_array($keys))
            die("Index could not be removed from MongoDB Collection because no keys were specified");
        if ($this->db->{$collection}->deleteIndex($keys, $options) == TRUE):
            $this->clear();
            return($this);
        else:
            die("An error occured when trying to remove an index from MongoDB Collection");
        endif;
    }
    ...
    public function remove_all_indexes($collection = "") {
        if (empty($collection))
            die("No Mongo collection specified to remove all indexes from");
    ...
    private function connect() {
        ...
        try {
            ...
            die("Unable to connect to MongoDB: {$e->getMessage()}");
        }
    }
     ...
     */
    private function connection_string($mongoConfig) {


        $this->host = trim($mongoConfig['host']);
        $this->port = trim($mongoConfig['port']);
        $this->user = trim($mongoConfig['user']);
        $this->pass = trim($mongoConfig['pass']);
        $this->dbname = trim($mongoConfig['dbname']);
        $this->persist = trim($mongoConfig['persist']);
        $this->persist_key = trim($mongoConfig['persist_key']);
