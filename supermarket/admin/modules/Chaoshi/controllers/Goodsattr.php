...
/**
 * @name GoodsattrController
 * @author Vic Shiwei
 * @desc 商品属性
 */
class GoodsattrController extends Core_Controller_Admin {
    
    protected $model;

    public function init() {
        parent::init();
        $this->model = new Chaoshi_GoodsattrModel();
    }
    ...
    public function editAction(){
        ...
        if(empty($gAttrInfo)){
            $this->redirect('/'.$this->_ModuleName.'/Goodsattr/index');
        }
    ...
    public function saveAction($data,$action){
        ...
        if ($saveR) {
            //保存成功跳转到列表页
            $this->ok(null, '/'.$this->_ModuleName.'/goodsattr/index/cateId/'.$data['goodsCateId'], '保存成功');
        } else {
            //返回来源地址;
