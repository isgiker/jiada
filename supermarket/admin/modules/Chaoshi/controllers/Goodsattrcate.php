 ...
 * @author Vic Shiwei
 */
class GoodsattrcateController extends Core_Controller_Admin {
    
    protected $model;

    public function init() {
        parent::init();
        $this->model = new Chaoshi_GoodsattrcateModel();
    }
    ...
    public function saveAction($data,$action){
        ...
        if ($saveR) {
            ...
            //保存成功跳转到列表页
            $this->ok(null, '/'.$this->_ModuleName.'/'.$this->_ControllerName.'/index/cateId/'.$data['goodsCateId'], $success_msg);
        } else {
